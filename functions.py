...
# Example usage:
# balanced_train_set = balance_dataset(train_set, label_column="label", seed=42)
...
def save_confusion_matrix(outputs, dataset_train, new_model_path, Map2Num):
    ...
    cm_percentage = cm.astype('float') / cm.sum(axis=1)[:, np.newaxis] * 100  # Compute percentages per class
...
class CustomDinatForImageClassification_V2(nn.Module):
    def __init__(self, base_model, num_classes, feature_dim, class_weights=None, alpha=1.0, beta=0.1, center_lr=0.5):
        ...
        # self.loss_fn = UAROptimizedContrastiveCenterLoss(
        #     num_classes=num_classes,
        #     feature_dim=feature_dim,
        #     alpha=alpha,     # Weight for cross-entropy
        #     beta=beta,      # Weight for contrastive center loss
        #     gamma=0.2,     # Weight for UAR optimization
        #     class_weights=class_weights,  # Optional
        #     margin=1.0     # Margin for better class separation
        # )
    ...
    def forward(self, pixel_values, labels=None, **kwargs):
        ...
        # Assume the penultimate layer is used for features
        features = outputs.hidden_states[-1]  # (B, feature_dim)
        # print(features.shape)  # Expected: (batch_size, feature_dim)
        features = features.mean(dim=(1, 2))  # Resulting shape: (batch_size, feature_dim)
...
class CombinedLoss(nn.Module):
    ...
    def forward(self, logits, features, targets):
        ...
        # Inside the CombinedLoss forward method:
        with torch.no_grad():
...
class UAROptimizedContrastiveCenterLoss(nn.Module):
    """
    A combination of cross-entropy, contrastive-center, and UAR-focused loss
    for multi-modal models with explicit optimization for Unweighted Average Recall.
    """
    def __init__(self,
                 num_classes: int,
                 feature_dim: int,
                 alpha: float = 0.4,    # Weight for cross-entropy loss
                 beta: float = 0.4,     # Weight for contrastive-center loss
                 gamma: float = 0.2,    # Weight for UAR optimization loss
                 class_weights: torch.Tensor = None,
                 margin: float = 1.0):  # Margin for better class separation
        """
        Args:
            ...
            alpha: Weight for cross-entropy loss.
            beta: Weight for contrastive-center loss.
            gamma: Weight for UAR-focused loss.
            class_weights: Optional class weights for cross-entropy loss.
            margin: Margin for contrastive center loss.
        """
        super(UAROptimizedContrastiveCenterLoss, self).__init__()
        self.num_classes = num_classes
        self.feature_dim = feature_dim
        self.alpha = alpha
        self.beta = beta
        ...
        # Register buffer for tracking per-class recall (for monitoring)
        self.register_buffer('class_recall', torch.zeros(num_classes))
    ...
    def forward(self, logits, combined_features, targets):
        ...
        Returns:
            Scalar loss combining cross-entropy, contrastive-center, and UAR-focused loss.
        """
        ...
        # Normalize features and centers for more stable distances
        ...
        centers_batch = normalized_centers[targets]  # (batch_size, feature_dim)
        
        # Compute intra-class distances (pull same-class samples closer)
        intra_class_distance = torch.norm(normalized_features - centers_batch, p=2, dim=1).mean()
        
        # Compute inter-class distances with margin
        center_distances = torch.norm(normalized_centers.unsqueeze(0) - normalized_centers.unsqueeze(1), p=2, dim=2)
        # Apply eye mask to ignore distance to self
        center_distances = center_distances + torch.eye(self.num_classes, device=center_distances.device) * 1e12
        # Apply margin to encourage minimum separation
        center_distances = torch.clamp(self.margin - center_distances, min=0.0)
        inter_class_distance = center_distances.sum(dim=1).mean() / (self.num_classes - 1)
        
        # Overall contrastive loss: minimize intra-distance and maximize inter-distance
        contrastive_center_loss = intra_class_distance + inter_class_distance
        
        # ---- 3. UAR-focused loss component ----
        # Compute per-class predictions for UAR estimation
        probs = F.softmax(logits, dim=1)
        pred_classes = torch.argmax(probs, dim=1)
        
        # Initialize tracking tensors for each class
        class_correct = torch.zeros(self.num_classes, device=device)
        class_total = torch.zeros(self.num_classes, device=device)
        
        # Count correct predictions per class in the current batch
        for c in range(self.num_classes):
            class_mask = (targets == c)
            if class_mask.sum() > 0:
                class_correct[c] = ((pred_classes == c) & class_mask).float().sum()
                class_total[c] = class_mask.float().sum()
        
        # Compute per-class recall and update running estimate
        current_recall = torch.where(class_total > 0, class_correct / class_total, torch.zeros_like(class_total))
        
        # Update running class recall estimate with exponential moving average
        with torch.no_grad():
            self.update_count += 1
            # Use small update rate for stability
            update_rate = min(0.1, 1.0 / self.update_count.item())
            # Update only for classes present in this batch
            for c in range(self.num_classes):
                if class_total[c] > 0:
                    self.class_recall[c] = (1 - update_rate) * self.class_recall[c] + update_rate * current_recall[c]
        
        # Create inverse recall weights (focus more on low-recall classes)
        # Use smoothed version to avoid division by zero
        smoothed_recall = torch.clamp(self.class_recall, min=0.01)
        inverse_recall_weights = 1.0 / smoothed_recall
        # Normalize weights
        inverse_recall_weights = F.normalize(inverse_recall_weights, p=1, dim=0) * self.num_classes
        
        # Apply class-weighted focal loss focusing on low-recall classes
        # Focal loss modulates the loss to focus on hard examples
        focal_gamma = 2.0  # Standard value for focal loss
        
        # Calculate focal loss with UAR optimization
        uar_loss = 0.0
        for c in range(self.num_classes):
            class_mask = (targets == c)
            if class_mask.sum() > 0:
                # Get probabilities for target class
                target_probs = probs[class_mask, c]
                # Apply focal weighting (1-p)^gamma
                focal_weights = (1 - target_probs) ** focal_gamma
                # Weight by inverse recall
                class_weight = inverse_recall_weights[c]
                # Compute loss for this class
                class_loss = -(focal_weights * torch.log(target_probs + 1e-10)).mean() * class_weight
                uar_loss += class_loss
        
        # Normalize by number of classes with samples in this batch
        uar_loss = uar_loss / (targets.unique().size(0))
        
        # ---- 4. Combine Losses ----
        total_loss = self.alpha * ce_loss + self.beta * contrastive_center_loss + self.gamma * uar_loss
        
        return total_loss
    
    # Optional method to get the current estimate of UAR
    def get_current_uar_estimate(self):
        """Returns the current estimate of UAR based on tracked class recalls."""
        valid_classes = (self.class_recall > 0).float().sum()
        if valid_classes > 0:
            return self.class_recall.sum() / valid_classes
        return torch.tensor(0.0, device=self.class_recall.device)
